...
First I tidied the data to make sure that all variables that were numeric were coded as numeric, and all categorical variables became factored variables. 

Afterwards, I used an automated stepwise fuction to generate a model of interest. This was using the `r stepAIC()` function that is part of the `r library(MASS)` package. From there, a model was generated to include the following variables: `r babysex`,  `r bhead` , `r blength` , `r delwt`, `r fincome`, `r gaweeks`, `r mheight`, `r mrace`, `r parity`, `r ppwt`, and `r smoken`. 
...
Looking at these graphs, my model, labeled "fit" appears to have the best fitting model. 